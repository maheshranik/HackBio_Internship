...
# Global summary of cholera cases and deaths
global_summary <- combined_data %>%
  group_by(Year) %>%
  summarize(Total_Cases = sum(Cases, na.rm = TRUE),
            Total_Deaths = sum(Deaths, na.rm = TRUE),
...
#generate Global summary for cholera cases over time
ggplot(global_summary, aes(x = Year, y = Total_Cases, color = "Total Cases")) +  # Include color inside aes for legend
  geom_line(size = 1.2) +
  labs(title = "Global Cholera Cases Over Time", x = "Year", y = "Total Cases") +
  scale_color_manual(values = c("Total Cases" = "blue")) +  # Manually set color for legend
  theme_minimal() +
  theme(legend.title = element_blank(), legend.position = "bottom")

#generate Global summary for cholera Deaths over time
ggplot(global_summary, aes(x = Year, y = Total_Deaths, color = "Total Deaths")) + 
  geom_line(size = 1.2) +
  labs(title = "Global Cholera Deaths Over Time", x = "Year", y = "Total Deaths") +
  scale_color_manual(values = c("Total Deaths" = "red")) + 
  theme_minimal() +
  theme(legend.title = element_blank(), legend.position = "bottom")

#generate Global summary for cholera CFR over time
ggplot(global_summary, aes(x = Year, y = Avg_CFR, color = "CFR")) + 
  geom_line(size = 1.2) +
  labs(title = "Global CFR Over Time", x = "Year", y = "CFR (%)") +
  scale_color_manual(values = c("CFR" = "green")) + 
  theme_minimal() +
  theme(legend.title = element_blank(), legend.position = "bottom")

# Function to create a country-specific report
create_country_report <- function(country_name) {
  ...
  # Plot Cholera Cases Over Time
  p1 <- ggplot(country_data, aes(x = Year, y = Total_Cases, color = "Total Cases")) +
    geom_line(size = 1.2) +
    labs(title = paste("Cholera Cases Over Time in", country_name), x = "Year", y = "Total Cases") +
    scale_color_manual(values = c("Total Cases" = "blue")) +
    theme_minimal() +
    theme(legend.title = element_blank(), legend.position = "bottom")
  
  # Plot Cholera Deaths Over Time
  p2 <- ggplot(country_data, aes(x = Year, y = Total_Deaths, color = "Total Deaths")) +
    geom_line(size = 1.2) +
    labs(title = paste("Cholera Deaths Over Time in", country_name), x = "Year", y = "Total Deaths") +
    scale_color_manual(values = c("Total Deaths" = "red")) +
    theme_minimal() +
    theme(legend.title = element_blank(), legend.position = "bottom")
  
  # Plot CFR Over Time
  p3 <- ggplot(country_data, aes(x = Year, y = Avg_CFR, color = "CFR")) +
    geom_line(size = 1.2) +
    labs(title = paste("CFR Over Time in", country_name), x = "Year", y = "CFR (%)") +
    scale_color_manual(values = c("CFR" = "green")) +
    theme_minimal() +
    theme(legend.title = element_blank(), legend.position = "bottom")
  
  # Display the plots and summary
  print(country_data)
  print(p1)
  print(p2)
  print(p3)
...
# Function to create a country-specific report with highest and lowest values for cases, deaths, and CFR
generate_report <- function(country_name) {
  # Filter and summarize the data for the selected country
  ...
  # Calculate the highest and lowest values for cases, deaths, and CFR
  max_cases <- max(country_summary$Total_Cases, na.rm = TRUE)
  min_cases <- min(country_summary$Total_Cases, na.rm = TRUE)
  max_deaths <- max(country_summary$Total_Deaths, na.rm = TRUE)
  min_deaths <- min(country_summary$Total_Deaths, na.rm = TRUE)
  max_cfr <- max(country_summary$Avg_CFR, na.rm = TRUE)
  min_cfr <- min(country_summary$Avg_CFR, na.rm = TRUE)
  
  # Find the years corresponding to the highest and lowest values
  year_max_cases <- country_summary$Year[which.max(country_summary$Total_Cases)]
  year_min_cases <- country_summary$Year[which.min(country_summary$Total_Cases)]
  year_max_deaths <- country_summary$Year[which.max(country_summary$Total_Deaths)]
  year_min_deaths <- country_summary$Year[which.min(country_summary$Total_Deaths)]
  year_max_cfr <- country_summary$Year[which.max(country_summary$Avg_CFR)]
  year_min_cfr <- country_summary$Year[which.min(country_summary$Avg_CFR)]
  
  # Print the summary for the country
  cat("### Cholera Outbreak Report for", country_name, "\n")
  cat("Reporting Period:", min(country_summary$Year), "to", max(country_summary$Year), "\n\n")
  
  # Highest and Lowest values for Cases, Deaths, and CFR
  cat("Total Cases Summary:\n")
  cat("Highest Cases:", max_cases, "in", year_max_cases, "\n")
  cat("Lowest Cases:", min_cases, "in", year_min_cases, "\n\n")
  
  cat("Total Deaths Summary:\n")
  cat("Highest Deaths:", max_deaths, "in", year_max_deaths, "\n")
  cat("Lowest Deaths:", min_deaths, "in", year_min_deaths, "\n\n")
  
  cat("CFR Summary (Case Fatality Rate):\n")
  cat("Highest CFR:", max_cfr, "% in", year_max_cfr, "\n")
  cat("Lowest CFR:", min_cfr, "% in", year_min_cfr, "\n\n")
  
  # Generate and display plots for the country
  create_country_report(country_name)
}

# Example usage: Generate a report for Haiti with highest and lowest values for cases, deaths, and CFR
generate_report("Nigeria")
